...
func (suite *EventsSuite) TestUpdaterConfirmPublishes() {
	...
	updater := usecase.NewUpdater(slogdiscard.NewDiscardLogger(), usecase.UpdaterRepositories{
		Marks:  marks,
		Checks: checks,
...
func (suite *EventsSuite) newChecks() (*usecase.Checks, checksMocks) {
	...
	updater := usecase.NewUpdater(log, usecase.UpdaterRepositories{Marks: m.marks, Checks: m.checks}).WithEvents(suite.publisher)
	uc := usecase.NewChecks(log, m.trm, updater, usecase.ChecksRepositories{
		Marks:  m.marks,
		Checks: m.checks,
		Tasks:  m.tasks,
		Photos: m.photos,
...
func (suite *EventsSuite) TestAddCheckPublishesAfterCommit() {
	...
	for _, tt := range tests {
		suite.Run(tt.name, func() {
			...
			m.marks.On("GetMarkById", mock.Anything, 5).Once().
				Return(models.Mark{ID: 5, UserID: 3, MarkStatusID: models.UnconfirmedStatus}, nil)
