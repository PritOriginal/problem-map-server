...
// to Closers.Add. The publish counters are registered on reg (nil: not
// registered).
func NewPublisher(log *slog.Logger, cfg config.NatsConfig, reg prometheus.Registerer) (events.Publisher, io.Closer) {
	if cfg.URL == "" {
		log.Warn("nats.url is empty: domain events are not published (notifications disabled)")
		return events.NoopPublisher{}, nil
	}

	client, err := nats.New(log, cfg, nats.WithMetrics(nats.NewMetrics(reg)))
	if err != nil {
