...
func (s *PostgresSuite) TestTasks_GetTasks() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			tasks, err := s.tasks.GetTasks(s.ctx, tt.filters)
...
func (s *PostgresSuite) TestTasks_GetTasksByUserId() {
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			tasks, err := s.tasks.GetTasksByUserId(s.ctx, tt.userID, tt.filters)
			s.Require().NoError(err)
			s.NotNil(tasks)
			s.ElementsMatch(tt.wantIDs, taskIDs(tasks))
		})
...
func (s *PostgresSuite) TestTasks_GetTaskByUserIdAndMarkId() {
	tests := []struct {
		...
		{name: "match", userID: fxUserBob, markID: fxMarkFar, wantID: fxTaskBobMark3},
		{name: "user has no task for mark", userID: fxUserBob, markID: fxMarkNear, wantErr: repository.ErrNotFound},
		{name: "unknown mark", userID: fxUserAlice, markID: 404, wantErr: repository.ErrNotFound},
	}

	for _, tt := range tests {
		s.Run(tt.name, func() {
			got, err := s.tasks.GetTaskByUserIdAndMarkId(s.ctx, tt.userID, tt.markID)
			if tt.wantErr != nil {
				s.ErrorIs(err, tt.wantErr)
				return
			}
