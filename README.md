...
Переводы хранятся в таблице `translations(entity, entity_id, lang, name)` (миграция `000037_add_i18n`); чтобы добавить язык, достаточно вставить строки с новым `lang`. Ответ помечается заголовками `Content-Language` и `Vary: Accept-Language`, а кэш справочников ведётся отдельно для каждого языка.
