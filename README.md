...
Параметры `reports.*` и `comments.*` задаются только конфигом (env/YAML) и
не входят в настройки времени выполнения `GET/PUT /admin/settings`
(`RuntimeSettings`): изменение требует перезапуска сервера.

| Метод | Путь | Кто | Что делает |
|---|---|---|---|
| `POST` | `/reports` | JWT | `{target_type: mark\|check\|comment, target_id, reason: spam\|offensive\|wrong_place\|duplicate\|other, comment?}` → `201`. Метка/проверка/комментарий должны существовать (`404`; удалённый комментарий считается отсутствующим) и не быть своими (`403`). Повторная жалоба на тот же объект — `409`, превышение суточного лимита — `429` |
...
- `POST /marks/{id}/comments {body, parent_id?}` — JWT; `parent_id`
  ...
  до 2000 символов. Скрытая метка и метка, слитая как дубликат
  (`merged_into_id`), комментариев не принимают — 409. Антиспам: тот же
  текст от того же пользователя на ту же метку в течение минуты — 409;
  больше `comments.max-per-day` (по умолчанию 100) комментариев за
  скользящие сутки — 429.
