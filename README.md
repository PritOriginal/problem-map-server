...
| `PATCH` | `/users/{id}/role` | admin | смена роли (`{"role": "user"|"moderator"|"admin"}`); сессии пользователя сбрасываются, новая роль действует сразу |
...
5 с), при расхождении — `401`. Так смена роли, пароля и `logout-all`
вступают в силу не дожидаясь истечения access-токена (с задержкой до 5 с).
...
После восстановления Redis refresh-токены, выданные во время простоя,
не найдутся в хранилище и будут отвергнуты (`401`, пользователю нужно войти
заново), а access-токены с устаревшим `ver` перестанут приниматься.
